...
public:
  ...
  {
    assert(!error_state_ && "Attempting to retrieve value when Result is in error!");
  ...
  {
    assert(!error_state_ && "Attempting to dereference a Result when it's in error!");
    return result_;// NOLINT
  }
  ...
  {
    assert(error_state_ && "Attempting to retrieve error when Result is NOT in error!");
