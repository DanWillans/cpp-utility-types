...
{
  cut::Error error("Uh oh, Error");
  cut::Result<TestClass> result{ error };
  REQUIRE(result.Bad());
...
{
  cut::Result<TestClass> result{ "Uh oh, error!" };
  REQUIRE(result.Bad());
...
TEST_CASE("Check result construction with const ResultType& argument")
{
  ...
  cut::Result<TestClass> result{ my_class };
  REQUIRE(result.Good());
...
{
  cut::Result<TestClass> result{ TestClass{ 1, 2.0, "Test" } };
  REQUIRE(result.Good());
  REQUIRE(result->a == 1);
  REQUIRE(result->b == 2.0);
  REQUIRE(strcmp(result->c, "Test") == 0);
  // Construct with {} instead.
  cut::Result<TestClass> result_2{ 1, 2.0, "Test" };
...
{
  cut::Result<TestClassList> result{ std::initializer_list<int>{ 0, 1, 2, 3, 4 } };
  REQUIRE(result.Good());
...
{
  cut::Result<TestClass> result{ 0, 1.0, "Test" };
  REQUIRE(result.Good());
  cut::Result<TestClass> copy_result{ result };
...
{
  cut::Result<TestClass> error_result{ "Uh oh, error!" };
  REQUIRE(error_result.Bad());
  cut::Result<TestClass> copy_error_result{ error_result };
...
{
  cut::Result<TestClassPtr> result{ 1 };
  REQUIRE(result.Good());
  cut::Result<TestClassPtr> move_result{ std::move(result) };
...
{
  cut::Result<TestClass> error{ cut::Error{ "Uh oh, error!" } };
  REQUIRE(error.Bad());
  cut::Result<TestClass> move_error{ std::move(error) };
